...
class DistilTrainer(BaseTrainer):
    ...
    def _generate_and_score_completions(
        ...
        # 2. 生成回复 (Generation)
        # 根据配置决定是用 Student Prompt 还是 Teacher Prompt 进行生成
        generation_prompts = teacher_prompts if self.generate_from_teacher else prompts
        
        # 调用 _generate 生成回复 (注意 images 传 None)
        (
            _,                      # 忽略 prompt_ids，因为我们要重新 tokenize
            completion_ids_list,    # 生成的回复 token ids (List[List[int]])
            num_items_in_batch,     # 用于 DAPO Loss 的统计
            sampling_per_token_logps_list, # 生成时的 logprobs (用于重要性采样)
            _                       # forward_kwargs (已忽略)
        ) = self._generate(generation_prompts, images=None)

        # 3. 处理 Student Prompts (Tokenize & Padding)
        # Student Prompt 需要左填充 (Left Padding) 以便对齐生成部分
        ...
        prompt_ids = student_inputs["input_ids"].to(device)
        prompt_mask = student_inputs["attention_mask"].to(device)

        # 4. 处理 Teacher Prompts (Tokenize & Padding)
        ...
        teacher_prompt_mask = teacher_inputs["attention_mask"].to(device)

        # 5. 处理生成的 Completions (Padding & Masking)
        # 将生成的 list 转为 tensor 并右填充 (Right Padding)
        completion_ids = [torch.tensor(ids, device=device) for ids in completion_ids_list]
        completion_ids = pad(completion_ids, padding_value=self.pad_token_id, padding_side="right")
        
        # 创建基础 Mask (非 padding 部分为 1)
        completion_mask = (completion_ids != self.pad_token_id).long()

# ==================== [修改开始] ====================
        # [修改] 截断逻辑：适配 <|endoftext|> (EOS Token)
        # 既然 Prompt 以 <|endoftext|> 结尾，模型也会生成这个 token 来停止
        
        # 1. 优先使用 tokenizer 定义的 eos_token_id
        stop_token_id = self.processing_class.eos_token_id
        
        # 2. 保底逻辑：万一 tokenizer 没配好，尝试手动找 <|endoftext|>
        if stop_token_id is None:
            stop_token_id = self.processing_class.convert_tokens_to_ids("<|endoftext|>")

        # 3. 开始计算截断 mask
        if isinstance(stop_token_id, int):
            # 找到 batch 中所有等于 stop_token 的位置
            is_stop_token = completion_ids == stop_token_id
            
            # 找到每行“第一个”出现的 stop_token 的索引
            first_stop_idx = is_stop_token.int().argmax(dim=1)
            
            # 标记哪些行确实包含了 stop_token
            has_stop_token = is_stop_token.any(dim=1)
            
            seq_len = completion_ids.size(1)
            # 生成索引矩阵 [0, 1, 2, ... seq_len-1]
            indices = torch.arange(seq_len, device=device).unsqueeze(0).expand(completion_ids.size(0), -1)
            
            # 确定截断点 cutoff_idx:
            # - 如果有 stop_token: 截断点就是该 token 的位置 (包含它自己，因为它也是要预测的目标)
            # - 如果没有 stop_token: 截断点设为 seq_len (全选)
            cutoff_idx = torch.where(has_stop_token, first_stop_idx, torch.tensor(seq_len, device=device))
            
            # 生成 Mask: 只有 index <= cutoff_idx 的位置是 1，后面全是 0
            truncation_mask = (indices <= cutoff_idx.unsqueeze(1)).int()
            
            # 将截断 Mask 应用到基础 Mask 上
            completion_mask = completion_mask * truncation_mask
        # ==================== [修改结束] ====================

        # 6. 计算参考概率 (Reference Logprobs)
        # 为了计算 KL 散度，我们需要知道 Reference Model (Teacher) 对这些生成的回复打多少分
        
        # 拼接 Teacher Prompt + Completion
        ...
        if self.accelerator.is_main_process:
            ...
            # 设置保存路径
            debug_save_dir = self.args.output_dir if self.args.output_dir else "."
            os.makedirs(debug_save_dir, exist_ok=True)
            debug_file = os.path.join(debug_save_dir, "debug_teacher_icl.jsonl")
            def clean_log(text):
                # 1. 把 <|pad|> 替换为空
                text = text.replace("<|pad|>", "")
                # 2. (可选) 去除首尾空白，避免日志开头一大堆空格
                return text.strip()
            # 解码 Teacher 的输入 (应该包含 System + Shots + Query)
            # 取 batch 的第 0 条数据
            raw_prompt = self.processing_class.decode(teacher_prompt_ids[0], skip_special_tokens=False)
            clean_prompt = clean_log(raw_prompt)  # <--- 清洗
            
            # 解码 Teacher 的输出
            raw_completion = self.processing_class.decode(completion_ids[0], skip_special_tokens=False)
            clean_completion = clean_log(raw_completion) # <--- 清洗
            
            # 解码 拼接后的全文
            raw_full = self.processing_class.decode(teacher_prompt_completion_ids[0], skip_special_tokens=False)
            clean_full = clean_log(raw_full) # <--- 清洗

            log_entry = {
                "step": self.state.global_step,
                "type": "teacher_forward_check",
                "teacher_prompt_input": clean_prompt,       
                "teacher_completion_output": clean_completion, 
                "full_concatenated": clean_full             
            }
        ...
        with torch.no_grad():
            if self.beta != 0.0: # 如果 beta=0 (不使用 KL)，则跳过
                # 使用 ref_model 计算 logprobs
                # 这里复用了 _get_per_token_logps_and_entropies 函数
                ref_per_token_logps, _, _ = self._get_per_token_logps_and_entropies(
                    self.ref_model if self.ref_model else self.model, # 如果没显式指定 ref_model，通常指 Teacher
            ...
            # 7. (可选) 处理重要性采样 (Importance Sampling)
            # 如果训练模型和生成模型不一致（例如使用了 vLLM 加速生成，或者梯度累积导致模型更新了），需要校正
            old_per_token_logps = None
            importance_sampling_ratio = None
            
            # 简单判断是否需要计算 old_logps (这里简化了判断逻辑，假设你需要)
            need_old_logps = not self.generate_from_teacher and (self.use_vllm or self.args.gradient_accumulation_steps > 1)
            
            if need_old_logps:
                # 计算 Student Model 对自己生成的回复的概率
                ...
                if sampling_per_token_logps_list is not None:
                    ...
                    sampling_logps = pad(sampling_logps, padding_value=0.0, padding_side="right")
                    
                    importance_sampling_ratio = torch.exp(old_per_token_logps - sampling_logps)
        ...
        if self.accelerator.is_main_process:
            ...
            # 定义保存路径，优先存在 output_dir 下，否则存在当前目录
            save_dir = self.args.output_dir if self.args.output_dir else "."
            os.makedirs(save_dir, exist_ok=True)
            log_file_path = os.path.join(save_dir, "debug_student_data.jsonl")
            def clean_log(text):
            ...
            # 遍历当前 Batch 的每条数据
            # 注意：如果 batch size 很大，这里可能会稍微拖慢一点速度，但为了 Debug 是值得的
            for i in range(len(prompt_ids)):
                # 1. 解码 Prompt (并清洗)
                raw_prompt = self.processing_class.decode(prompt_ids[i], skip_special_tokens=False)
                prompt_text = clean_log(raw_prompt)
                
                # 2. 解码完整的 Completion
                raw_full = self.processing_class.decode(completion_ids[i], skip_special_tokens=False)
                full_completion_text = clean_log(raw_full)
                
                # 3. 解码有效的 Completion
                mask_row = completion_mask[i].bool()
                valid_tokens = completion_ids[i][mask_row]
                valid_completion_text = clean_log(self.processing_class.decode(valid_tokens, skip_special_tokens=False))
                
                # 4. 解码被丢弃的部分
                ignored_tokens = completion_ids[i][~mask_row]
                ignored_completion_text = clean_log(self.processing_class.decode(ignored_tokens, skip_special_tokens=False))

                logs_to_save.append({
                    "step": self.state.global_step,
                    "index": i,
                    "prompt": prompt_text,                   # <--- 现在这里会很清爽，直接是 <|im_start|> 开头
                    "completion_full": full_completion_text, 
                    "completion_valid": valid_completion_text, 
                    "completion_ignored": ignored_completion_text, 
                    "is_truncated": bool(len(ignored_tokens) > 0) 
                })

            # 追加写入
            with open(log_file_path, "a", encoding="utf-8") as f:
                for entry in logs_to_save:
        ...
        return {
            ...
            "advantages": torch.zeros_like(completion_ids, dtype=torch.float32) # 占位符，保持兼容
        }
